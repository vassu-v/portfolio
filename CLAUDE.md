...
1. Add an entry to `PROJECTS` in `src/data/projects.js` — `name`, `tagline`, `tags`, `desc`, `stat`, `github`/`live`, `period`, `problem`/`solution`/`how`/`impact` content, and `images: [{ src, alt }, ...]` (alt text is real and hand-written per image, not derivable — see below).
...
**Two fields that aren't single-sourced — both SEO-curated by hand, neither derivable from other fields:**
