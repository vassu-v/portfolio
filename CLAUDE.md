...
## CLI mode (curl + web terminal)

A "curl my site" easter egg plus a matching browser-based fake terminal, deliberately kept **cleanly separated** from the main React app — this is the one part of the codebase that is NOT single-sourced from `src/data/`, and that's a real, known limitation, not an oversight.

**Where it lives:**
- `api/cli.py` — a FastAPI app. `vercel.json` routes any request whose `User-Agent` header matches `curl` (for `/`, `/whoami`, `/projects`, `/blog`, `/contact`, `/animate`) to this function instead of the real site; everyone else's request falls through to the normal static SPA untouched. Renders ANSI-colored terminal-style text (box-drawing panels, a 60/30/10 color hierarchy — copper is a deliberate *minority* accent for the banner/prompt only, not the default for every label).
- `public/terminal/index.html` + `terminal-engine.js` + `terminal-ui.js` — a **standalone static page**, plain HTML/CSS/vanilla JS, zero build step, zero framework, reachable at `/terminal/index.html`. `terminal-engine.js` is a pure `processCommand(state, command)` function (verified byte-for-byte content match against a Python prototype during development) simulating a fake filesystem (`ls`/`cd`/`cat`/`nano`/`whoami`/`sudo`/etc.); `terminal-ui.js` is the only file that touches the DOM.
- Integration touchpoints in the real app — intentionally tiny, three one-liners, nothing more: `src/App.jsx`'s `Preloader` has a `<a href="/terminal/index.html">switch to CLI mode</a>` link; `src/components/Highlights.jsx`'s `CRTCard` navigates there on click; `src/three/DeskScene.jsx`'s laptop-screen mesh has the same `onClick`, but **it currently cannot fire** — the whole 3D scene sits behind a site-wide `pointer-events: none` wrapper by design (so the decorative background never steals clicks from real page content), and fixing that would mean touching shared pointer-events/z-index layering used everywhere, which hasn't been done. `src/components/Footer.jsx`'s `CurlHint` button copies `curl https://shoryavardhaan.vercel.app` to the clipboard.

**⚠️ Content here is a manually-ported SNAPSHOT, not generated from `src/data/`.** Unlike the blog/project static pages above, `api/cli.py`'s `WHOAMI_PLAIN`/`render_whoami()`/`PROJECTS`/`BLOG_POSTS` and `terminal-engine.js`'s `ABOUT_MD`/work-history/project-README/blog-excerpt strings are **hand-copied text**, not derived from `src/data/blog.js`, `src/data/projects.js`, or `src/components/Experience.jsx` at build or runtime. If you edit a real post, project, or work-history entry, the CLI/terminal versions do **not** update automatically — they will silently drift out of sync exactly like the old hand-written static pages this project already moved away from once (see "Static blog + project pages" above). There is no generator script for this yet. Until one exists, treat any edit to `blog.js`/`projects.js`/`Experience.jsx` as also requiring a manual pass over `api/cli.py` and `terminal-engine.js` to keep them honest.

**Never hardcode a literal age number in these files.** The live SPA computes age dynamically (`src/utils/meta.js`'s `AGE`), but these are static files with no build step — a literal "17" here will be silently wrong within a year (this has already happened and been fixed twice). Describe the person without a number ("student developer") instead.

**~30 hardcoded absolute URLs to `https://shoryavardhaan.vercel.app`** across `api/cli.py`, `terminal-engine.js`, and `index.html` (self-referential links, `og:url`/canonical/JSON-LD `@id`, the footer curl-hint). Vercel keeps the `*.vercel.app` domain working even after a custom domain is added, so nothing breaks — but these won't automatically switch to a new custom domain either. If one gets added later, these three files need a manual find-and-replace pass.
