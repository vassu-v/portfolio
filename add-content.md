...
6. Run `npm run generate:blog` — generates the static, crawler-visible `public/blog/your-slug/index.html`. Then run `npm run generate:projects` too — it's the one that re-syncs `public/sitemap.xml` (see CLAUDE.md "Static blog + project pages")
